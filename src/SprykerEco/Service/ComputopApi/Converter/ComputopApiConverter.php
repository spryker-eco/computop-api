...
class ComputopApiConverter implements ComputopApiConverterInterface
{
    ...
    {
        $decryptedArray = [];
        $decryptedSubArray = (array)explode($this->computopApiConfig->getDataSeparator(), $decryptedString);
        foreach ($decryptedSubArray as $value) {
            $data = (array)explode($this->computopApiConfig->getDataSubSeparator(), (string)$value);
